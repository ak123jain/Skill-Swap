    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black p-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-12">
          <h2 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-red-500 to-purple-600">
            Community Members
          </h2>
          <button
            onClick={fetchUsers}
            className="group relative flex items-center gap-2 px-5 py-2.5 bg-black rounded-full border border-gray-800 text-white hover:border-purple-500 transition-all duration-300"
          >
            <div className="absolute inset-0 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 opacity-0 group-hover:opacity-20 blur-sm transition-opacity duration-300"></div>
            <RefreshCw size={16} />
            <span>Refresh</span>
          </button>
        </div>
        ...
        <div className="flex justify-center mb-10">
          <div className="bg-gray-800 rounded-full p-1 flex w-64">
            <button className="py-2 px-6 rounded-full text-white bg-gradient-to-r from-purple-600 to-pink-600 w-32 text-center font-medium">
              All Users
            </button>
        ...
        <div className="mb-12 max-w-lg mx-auto">
          <div className="relative flex items-center">
            ...
            <input
              ...
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-12 pr-10 py-4 rounded-full bg-gray-800/60 text-white border border-gray-700 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all shadow-lg"
            />
            {searchTerm && (
              <button
                onClick={clearSearch}
        ...
        {loading ? (
          <div className="flex justify-center items-center p-16">
            <div className="relative w-16 h-16">
              <div className="absolute top-0 left-0 w-full h-full rounded-full border-4 border-l-transparent border-r-transparent border-t-purple-500 border-b-pink-500 animate-spin"></div>
              <div className="absolute top-0 left-0 w-full h-full rounded-full border-4 border-l-pink-500 border-r-purple-500 border-t-transparent border-b-transparent animate-spin animation-delay-500"></div>
            </div>
          </div>
        ) : error ? (
          ...
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {filteredUsers.map((user) => (
              <div
                key={user._id}
                className="relative group bg-gray-900/80 rounded-xl overflow-hidden shadow-xl border border-gray-800 transition-all duration-300"
                onMouseEnter={() => setHoveredCard(user._id)}
                onMouseLeave={() => setHoveredCard(null)}
              >
                <div className={`absolute inset-0 rounded-xl bg-gradient-to-r from-orange-500 to-pink-600 opacity-0 blur-xl transition-opacity duration-700 ${hoveredCard === user._id ? 'opacity-20' : ''}`}></div>
                <div className={`absolute inset-0 rounded-xl bg-gradient-to-r from-orange-500 to-pink-600 opacity-0 transition-opacity duration-500 ${hoveredCard === user._id ? 'opacity-100' : ''}`} style={{ padding: '1px', margin: '-1px' }}></div>

                <div className="relative z-10 h-20 bg-gradient-to-r from-orange-500 to-pink-600"></div>
                <div className="relative z-10 px-6 pb-6 pt-0">
                  ...
                  <div className="bg-gray-800/80 rounded-xl p-4 mb-5">
                  ...
                  <div className="flex justify-between items-center">
                    {hoveredCard === user._id ? (
                      <div className="relative p-0.5 rounded-full overflow-hidden">
                        <div className="absolute inset-0 bg-gradient-to-r from-orange-500 via-red-500 to-pink-600 animate-spin-slow opacity-90"></div>
                        <button className="relative flex items-center justify-center gap-2 py-2 px-6 text-sm font-medium text-white bg-gray-900 rounded-full z-10">
                      ...
                      <div className="flex gap-3">
                        <button className="flex items-center justify-center w-9 h-9 rounded-full bg-gray-800 text-gray-400 hover:text-pink-400 transition-colors">
                    ...
                    <div
                      className={`relative py-2 px-6 rounded-2xl overflow-hidden ${hoveredCard === user._id ? 'bg-gradient-to-r from-orange-500 to-pink-600' : 'bg-gray-800'} transition-all duration-300`}
                    >
                      {hoveredCard === user._id && (
                        <div className="absolute inset-0 bg-gradient-to-r from-orange-500 to-pink-600 blur-md"></div>
                      )}
                      <div className="relative z-10 flex items-center gap-1">
                        <span className={`text-sm font-medium ${hoveredCard === user._id ? 'text-white' : 'text-gray-300'}`}>
                          {hoveredCard === user._id ? 'View Profile' : 'Profile'}
                        </span>
                        <ChevronRight size={16} className={hoveredCard === user._id ? 'text-white' : 'text-gray-400'} />
                      </div>
                    </div>
                  </div>
                </div>
